...
var RatelimitOptionKeys = ["key", "limit", "period"],
	RatelimitPeriodValues = [10, 60];
...
var Ratelimit = class {
	...
	async limit(options) {
		...
		let { key = "", limit = this.limitVal, period = this.period } = options;
		validate(typeof key == "string", `invalid key: ${key}`),
			...
			validate(RatelimitPeriodValues.includes(period), `unsupported period: ${period}`);
...
export { ratelimit_worker_default as default };
