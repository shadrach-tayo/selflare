	...
	"../../node_modules/.pnpm/@sentry+utils@7.76.0/node_modules/@sentry/utils/cjs/misc.js"(
		...
		function uuid4() {
			let gbl = worldwide.GLOBAL_OBJ,
				crypto = gbl.crypto || gbl.msCrypto,
...
var src_default = {
	async fetch(request, env, ctx) {
		let sentry,
			analytics = new Analytics(env.ANALYTICS),
			performance = new PerformanceTimer(env.UNSAFE_PERFORMANCE),
		...
		try {
			...
			return env.CONFIG.has_user_worker
				? (await env.ASSET_WORKER.unstable_canFetch(request))
					? (analytics.setData({ dispatchtype: "asset" /* ASSETS */ }),
						await env.ASSET_WORKER.fetch(maybeSecondRequest))
					: (analytics.setData({ dispatchtype: "worker" /* WORKER */ }),
						env.USER_WORKER.fetch(maybeSecondRequest))
				: (analytics.setData({ dispatchtype: "asset" /* ASSETS */ }),
					await env.ASSET_WORKER.fetch(request));
		} catch (err) {
			throw (
...
export { router_worker_default as default };
